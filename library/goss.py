...
options:
    ...
    sleep:
        required: false
        description:
            - Change goss sleep time.
            - Default: 1s
    timeout:
        required: false
        description:
            - Change goss retry timeout.
            - Default: 0
    output_file:
        required: false
        description:
...
def check(module, test_file_path, test_output_format, test_sleep, test_retry_timeout):
    cmd = "goss"
    if test_file_path != None:
        cmd += " -g {0}".format(test_file_path)
    cmd += " v"
    if test_output_format != None:
        cmd += " -f {0}".format(test_output_format)
    if test_sleep != None:
        cmd += " -s {0}".format(test_sleep)
    if test_retry_timeout != None:
        cmd += " -r {0}".format(test_retry_timeout)
...
def main():
    module = AnsibleModule(
        argument_spec = dict(
            ...
            sleep = dict(required=False, type='str'),
    ...
    test_output_format = module.params['format'] # goss output format
    test_sleep = module.params['sleep'] # test sleep
    test_retry_timeout = module.params['timeout'] # test retry timeout
    ...
    (rc, out, err) = check(module, test_file_path, test_output_format, test_sleep, test_retry_timeout)
